...
if (isset($_POST["post"])) {
    ...
    $sql = "INSERT INTO food_review(rev_name,email,username,user_id,food_id,rating,comment,date) VALUES (?,?,?,?,?,?,?,?)";
    $stmtinsert = $connect->prepare($sql);
    if ($stmtinsert) {
        $result = $stmtinsert->execute([$name, $email, $username, $user_id, $food_id, $rating, $comment, $date]);
        if ($result) {
            $call = "1";
        }else{
